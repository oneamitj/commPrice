...
def main(data):
	...
	# print 'The predicted price of %s on %s is %f' % (str(data).replace("dataset.",''), str(num2date(date_x))[:11], price_x)
	Title = "Regression line and original data plot of %s" % (str(data).replace("dataset.",''))

	fig, ax = plt.subplots()
	ax.set_title(Title)
	# plt.plot_date(date_x, price_x, color='red')
	plt.plot_date(dates, price, color='lightgreen')
	plt.plot_date(x, y, color='pink')
...
if choice == 1:
	...
	main(Garlic_Dry_Chinese)

elif choice == 33:
	main(Bauhania_flower)
