...
def main(data):
	...
	Title = "Regression fit (green) and original data plot (blue) of %s" % (str(data).replace("dataset.",''))

	fig, ax = plt.subplots()
	ax.set_title(Title)
	plt.plot_date(dates, price, '-', color='lightgreen', label='Regression Fit Line', markersize=10)
	plt.plot_date(x, y, '.', color='blue', markersize=10)
	...
	if qstn == 'y':
		...
		plt.plot_date(date_x, price_x, 'o', color='red', markersize=10)
